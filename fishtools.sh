...
press_any_key() { echo ""; read -n 1 -s -r -p "按任意键返回主菜单..." </dev/tty; }
...
# 子菜单: 性能/网络测试脚本
show_test_menu() {
    # 这里的逻辑和 optimization 菜单一样，也需要优化
    while true; do
        echo ""
        echo "=========== 性能/网络测试子菜单 ==========="
...
deploy_preset_project() {
    ...
    local url_yaml="https://raw.githubusercontent.com/qqzhoufan/fishtools/main/presets/${project_name}/docker-compose.yaml"
    local url_yml="https://raw.githubusercontent.com/qqzhoufan/fishtools/main/presets/${project_name}/docker-compose.yml"
    clear
    log_info "即将部署精选项目: ${project_name}"; log_info "目标目录: ${project_dir}"; echo ""
...
show_preset_deployment_menu() {
    while true; do
        ...
        echo "1. Portainer-CE (Docker管理面板)"; echo "2. Homepage (精美起始页)"; echo "3. AdGuard-Home (去广告DNS)"; echo "4. Nginx-Proxy-Manager (Nginx反代神器)"
        echo "----------------------------------------------"; echo "0. 返回上一级菜单"; echo "=============================================="
        read -p "请选择您要部署的项目: " preset_choice </dev/tty
        case $preset_choice in
            1) deploy_preset_project "portainer-ce" ;; 2) deploy_preset_project "homepage" ;; 3) deploy_preset_project "adguard-home" ;; 4) deploy_preset_project "nginx-proxy-manager" ;; 0) break ;; *) log_error "无效输入。"; press_any_key ;;
        esac
...
show_deployment_menu() {
    while true; do
        ...
        echo "1. 一键部署精选项目 (推荐)"; echo "2. 从自定义GitHub仓库部署 (高级)"; echo "0. 返回主菜单"
        echo "==========================================="
...
main() {
    while true; do
        clear
        echo "================================================="
        echo "      欢迎使用 fishtools by 咸鱼银河 v3.3"
        echo "================================================="
        echo "1. 系统状态监控"
        echo "2. 性能/网络测试"
        echo "3. DD系统/重装系统"
