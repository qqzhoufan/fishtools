...
# 核心修复：为 press_any_key 中的 read 命令增加 < /dev/tty
press_any_key() { read -n 1 -s -r -p "按任意键返回..." </dev/tty; }
    ...
    while true; do
        ...
        case $install_choice in
            1)
                ...
                if ! command -v docker &> /dev/null; then curl -fsSL https://get.docker.com | bash; sudo usermod -aG docker "$USER"; log_success "Docker 安装成功。"; else log_success "Docker 已安装。"; fi
                if ! docker compose version &> /dev/null; then sudo apt-get update; sudo apt-get install -y docker-compose-plugin; log_success "Docker Compose 插件安装成功。"; else log_success "Docker Compose 已安装。"; fi
                press_any_key ;;
            2) log_info "正在安装 Nginx..."; sudo apt-get update && sudo apt-get install -y nginx; log_success "Nginx 安装完成。"; press_any_key ;;
            3) log_info "正在安装 Caddy..."; sudo apt-get install -y debian-keyring debian-archive-keyring apt-transport-https; curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | sudo gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg; curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt' | sudo tee /etc/apt/sources.list.d/caddy-stable.list >/dev/null; sudo apt-get update && sudo apt-get install -y caddy; log_success "Caddy 安装完成。"; press_any_key ;;
            0) break ;;
...
show_test_menu() {
    while true; do
        ...
        case $test_choice in
            1) log_warning "即将执行 Superbench 脚本..."; curl -Lso- https://down.vpsaff.net/superbench/superbench.sh | bash; press_any_key ;;
            2) log_warning "即将执行 Lemonbench 脚本..."; curl -fsL https://ilemonra.in/LemonBenchIntl | bash -s fast; press_any_key ;;
            0) break ;;
            *) log_error "无效输入。"; press_any_key ;;
        esac
    done
...
show_dd_menu() {
    while true; do
        ...
        case $dd_choice in
            1) log_error "功能待实现！请在脚本中填入您的命令。"; press_any_key ;;
            2) log_error "功能待实现！请在脚本中填入您的命令。"; press_any_key ;;
            0) break ;;
            *) log_error "无效输入。"; press_any_key ;;
        esac
    done
...
deploy_preset_project() {
    ...
    if [[ -z "$project_name" ]]; then log_error "内部错误：项目名不能为空。"; return; fi
    ...
    clear
    log_info "即将部署精选项目: ${project_name}"; log_info "目标目录: ${project_dir}"; log_info "配置文件URL: ${compose_url}"; echo ""
    if ! command -v docker &> /dev/null || ! docker compose version &> /dev/null; then log_error "Docker 或 Docker Compose 未安装。请先从主菜单安装。"; return; fi
    read -p "确认部署? (y/n): " confirm </dev/tty
    if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then log_info "操作已取消。"; return; fi
    log_info "正在创建项目目录: ${project_dir}"; sudo mkdir -p "$project_dir"
    log_info "正在从 ${AUTHOR_GITHUB_USER} 的仓库下载 docker-compose.yml ...";
    if sudo curl -sL -o "${project_dir}/docker-compose.yml" "$compose_url"; then
        if [[ ! -s "${project_dir}/docker-compose.yml" ]]; then log_error "下载失败或文件为空！请检查项目名是否正确。"; sudo rm -rf "$project_dir"; return; fi
        log_success "docker-compose.yml 下载成功。"
    else
        log_error "下载失败！请检查网络或URL。"; return;
    fi
    log_info "进入目录并使用 Docker Compose 启动项目..."; cd "$project_dir" || return
    sudo docker compose up -d
    if [[ $? -eq 0 ]]; then log_success "项目 '$project_name' 已成功部署！"; else log_error "项目部署失败！"; fi
}
...
show_preset_deployment_menu() {
    while true; do
        ...
        echo "1. Portainer-CE (强大的Docker管理面板)"
        echo "2. Homepage (精美起始页)"
        echo "3. AdGuard-Home (去广告DNS服务)"
        ...
        if [[ "$preset_choice" -ne "0" ]]; then press_any_key; fi
    done
}

# 子菜单：从自定义仓库部署
deploy_from_custom_repo() {
    clear
    log_warning "此功能为高级功能，需要您fork本项目并修改脚本顶部的配置变量。";
    log_error "功能占位，暂未实现。"
    press_any_key
}
...
main() {
    while true; do
        clear
        echo "================================================="
        echo "      欢迎使用 fishtools by 咸鱼银河 v2.6"
        echo "================================================="
        echo "1. 显示VPS基本信息"
        echo "2. 性能/网络测试脚本 (Bench Scripts)"
        echo "3. DD系统/重装系统 (Reinstall OS)"
        echo "4. 显示VPS实时性能"
        echo "5. 常用软件安装"
